# class GroongaDevSearch::Web::App
#
  ...
  module Web
    ...
    class App < Sinatra::Base
      ...
      get "/search.json" do
        ...
        json posts.collect {|post| post.attributes }
      end
      ...
      post "/similar" do
        ...
        text = params[:text] || ""
        @posts = searcher.similar_search_by_text(database, text).take(50)
        haml :similar
      end
      ...
      helpers do
        def search_and_paginate
          ...
          database.open(Command.new.database_dir)
          searcher = GroongaSearcher.new
          @posts = searcher.search(database, words, options)
          @snippet = searcher.snippet
          page = (params[:page] || 1).to_i
          size = (params[:n_per_page] || 20).to_i
          begin
            @paginated_posts = @posts.paginate([["_score", :desc]],
                                               page: page,
                                               size: size)
          ...
          @paginated_posts.extend(PaginationProxy)
          @paginated_posts
        end
        ...
        def snippets
          ...
          snippet.execute(selected_posts.first.content)
        end
        ...
        def link(post)
          "https://lists.osdn.me/mailman/archives/groonga-dev/#{post._key.slice(/\A\d+-\w+/)}/thread.html"
        end
      end
    end
  end
